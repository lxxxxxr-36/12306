...
### 2.3 下单与锁票（当前实现说明）
- 流程：选择车次→在订单确认页选择乘客与席别→提交订单→支付→出票。
- 订单提交即扣减库存：调用 `web/src/services/trains.ts:55` 的 `decrementInventory(trainCode, seatType, count)` 扣减所选席别余票。
- 座位分配：在支付时随机分配车厢与座位号（`web/src/services/orders.ts:49` 的 `payOrder`），订单创建阶段仅确认席别与价格。
- 时效与并发：当前版本未实现支付时限与并发锁；无队列/超时释放逻辑。
- 规则校验：距发车不足30分钟禁止提交订单（`web/src/pages/Results.tsx:176`；`web/src/pages/ConfirmOrder.tsx:78`）；学生价按 0.9 折计算（`web/src/pages/ConfirmOrder.tsx:72`–`web/src/pages/ConfirmOrder.tsx:75`）。
...
### 2.5 订单中心与退改签（当前实现说明）
- 订单状态：`pending`（待支付）、`paid`（已支付）、`refunding`（退票中）、`cancelled`（已取消）。页面入口 `web/src/pages/Orders.tsx:73`。
- 去支付：待支付订单点击“去支付”进入 `/checkout/{order_id}`（`web/src/pages/Orders.tsx:112`），在确认页点击“立即支付”完成出票（`web/src/pages/ConfirmOrder.tsx:369`）。
- 退票：距发车不足1小时禁止退票（`web/src/services/orders.ts:82`），成功后恢复库存并标记取消（`web/src/services/orders.ts:90`）。
- 变更到站：待支付或已支付且未发车可变更到达地（`web/src/services/orders.ts:104`–`web/src/services/orders.ts:127`）；在列表页内嵌编辑（`web/src/pages/Orders.tsx:112` 之后的内嵌区）。
- 改签（改日期）：待支付或已支付且未发车可修改出发日期，且不得早于当天（`web/src/services/orders.ts:129`–`web/src/services/orders.ts:158`）；在列表页内嵌编辑（`web/src/pages/Orders.tsx:112` 之后的内嵌区）。
...
- 受让人：本人默认受让人；受让人添加上限为 8 人（`web/src/pages/my/sections/BeneficiariesList.tsx:35`；服务层限制 `web/src/services/beneficiaries.ts:78`）。
...
### 9.6 购票（对齐当前代码）
- 路由与入口：
  - 查询结果页 → `/results?origin={city}&dest={city}&date={YYYY-MM-DD}&search=1`（`web/src/pages/Results.tsx:269`–`web/src/pages/Results.tsx:279`）
  - 订单确认（新建）→ `/checkout/new?origin={city}&dest={city}&date={YYYY-MM-DD}&train={code}&stu=0|1`，来自查询页“预订”按钮（`web/src/pages/Results.tsx:489`）
  - 支付确认 → `/checkout/{order_id}`（订单中心“去支付”导航，`web/src/pages/Orders.tsx:112`；路由定义 `web/src/App.tsx:280`）
- 受保护与跳转：
  - `/checkout/:id` 与 `/orders` 为受保护路由，未登录跳转 `/login` 并带上来源路径（`web/src/App.tsx:279`–`web/src/App.tsx:283`）
- 查询结果页要点：
  - 筛选与排序：发车时段、车次类型（G/D）、席别过滤、价格/时间排序（`web/src/pages/Results.tsx:170`–`web/src/pages/Results.tsx:214`）
  - 仅看有票：勾选后同时隐藏距发车不足30分钟的车次（`web/src/pages/Results.tsx:175`–`web/src/pages/Results.tsx:181`）
  - 演示数据：覆盖 0–23 点每小时至少一班高铁（`web/src/services/trains.ts:23`–`web/src/services/trains.ts:33`）
- 订单确认页（`/checkout/new`）结构与交互：
  - 乘客来源分组：受让人与乘车人分组展示，支持按姓名过滤（`web/src/pages/ConfirmOrder.tsx:101`–`web/src/pages/ConfirmOrder.tsx:142`）
  - 混选限制：受让人与乘车人不可混选（勾选跨分组会提示“不支持混合选择”，`web/src/pages/ConfirmOrder.tsx:114`–`web/src/pages/ConfirmOrder.tsx:118`；`web/src/pages/ConfirmOrder.tsx:152`–`web/src/pages/ConfirmOrder.tsx:155`）
  - 证件类型与席别：行内选择证件类型（下拉八类证件）与席别（依据库存禁用选项，`web/src/pages/ConfirmOrder.tsx:199`–`web/src/pages/ConfirmOrder.tsx:227`）
  - 提交订单：点击“提交订单”弹出座位选择模态；每位乘客可选座位字母（A/B/C/D/F），不选则随机分配（`web/src/pages/ConfirmOrder.tsx:259`–`web/src/pages/ConfirmOrder.tsx:320`）
  - 学生价：价格计算对学生票按 0.9 折（`web/src/pages/ConfirmOrder.tsx:72`–`web/src/pages/ConfirmOrder.tsx:75`）
  - 售票截止：距发车不足30分钟按钮置灰并提示（`web/src/pages/ConfirmOrder.tsx:78`–`web/src/pages/ConfirmOrder.tsx:80`，按钮属性 `title` 与 `disabled`）
- 订单创建与库存（提交模态“确认”后）：
  - 创建订单：写入本地 `orders` 并派发 `orderschange` 事件（`web/src/services/orders.ts:20`–`web/src/services/orders.ts:42`）
  - 扣减库存：按所选席别与人数调用 `decrementInventory`（`web/src/pages/ConfirmOrder.tsx:312`；实现 `web/src/services/trains.ts:55`–`web/src/services/trains.ts:69`）
  - 跳转：关闭模态并进入订单中心（`web/src/pages/ConfirmOrder.tsx:313`–`web/src/pages/ConfirmOrder.tsx:314`）
- 支付与出票（`/checkout/{order_id}`）：
  - 立即支付：调用 `payOrder` 随机分配车厢与座位号（`web/src/pages/ConfirmOrder.tsx:338`；实现 `web/src/services/orders.ts:49`–`web/src/services/orders.ts:66`）
  - 退票：距发车不足1小时不可退（`web/src/pages/ConfirmOrder.tsx:340`）；成功后恢复库存并标记取消（`web/src/services/orders.ts:87`–`web/src/services/orders.ts:96`）
- 验收要点：
  - 查询→确认（含座位选择）→支付全链路可用；库存扣减与退票恢复一致；混选限制与售票截止生效。

### 9.7 候补购票（对齐当前代码）
- 路由与入口：
  - 候补购票页 → `/standby`（顶部导航进入，`web/src/App.tsx:115`–`web/src/App.tsx:118`）
- 登录与跳转：
  - 未登录提交候补时跳转 `/login` 并附带来源路径（`web/src/pages/Standby.tsx:77`–`web/src/pages/Standby.tsx:85`）
- 页面结构与交互：
  - 顶部行程卡片：出发地/目的地下拉（热门城市），日期最小为当天（`web/src/pages/Standby.tsx:118`–`web/src/pages/Standby.tsx:147`）
  - 车次选择：根据行程与日期联动加载，显示车次号与时刻（`web/src/pages/Standby.tsx:160`–`web/src/pages/Standby.tsx:175`）
  - 乘客信息：姓名/证件类型/证件号输入，支持多位乘客（`web/src/pages/Standby.tsx:174`–`web/src/pages/Standby.tsx:189`）
  - 席别偏好：可多选 `商务/一等/二等/无座`（`web/src/pages/Standby.tsx:190`–`web/src/pages/Standby.tsx:198`）
  - 截止与优先级：候补截止（2h/6h/24h）与优先级（时间/价格）（`web/src/pages/Standby.tsx:199`–`web/src/pages/Standby.tsx:207`）
  - 定金（演示）：按每位乘客 ¥50 计算（`web/src/pages/Standby.tsx:210`–`web/src/pages/Standby.tsx:216`）
- 校验与限制：
  - 城市必选且不可相同、日期不得早于今天（`web/src/pages/Standby.tsx:86`–`web/src/pages/Standby.tsx:101`）
  - 距发车不足30分钟禁止提交候补（`web/src/pages/Standby.tsx:91`–`web/src/pages/Standby.tsx:99`）
- 状态流转与存储：
  - 创建候补请求：`createStandby(payload)` 返回 `id/status` 并持久化到 `localStorage`（`web/src/services/standby.ts:17`、`web/src/services/standby.ts:42`）
  - 状态检查：`matching` → `success|expired|cancelled`；成功时从偏好中选定席别（`web/src/services/standby.ts:71`–`web/src/services/standby.ts:98`）
- 验收要点：
  - 行程/日期联动正常、提交校验完整；状态变化与文案展示正确；未登录跳转后返回来源。
...
### 9.17 会员页面与样式（新增）
- 路由：`/membership`（未登录可访问）
- 结构与文件：
  - 页面组件：`web/src/pages/Membership.tsx`
  - 样式文件：`web/src/pages/membership.css`
- 侧边栏与背景对齐：
  - 统一高度为 `340px`，背景容器 `member-hero` 与覆盖层 `member-overlay` 高度一致（`web/src/pages/membership.css:8`, `web/src/pages/membership.css:20`）。
  - 侧边栏 `member-sidebar` 使用 `height: 100%` 与 `box-sizing: border-box`，保证包含内边距不溢出（`web/src/pages/membership.css:23-33`）。
- 悬停面板文案横排：
  - `hover-panel` 与 `hover-item` 设置 `white-space: nowrap` 与 `writing-mode: horizontal-tb`，避免逐字换行（`web/src/pages/membership.css:50-51`）。
- 会员页导航交互：
  - 悬停某主项展示二级子项；点击二级子项导航至 `member` 中的对应子页面，URL 形如 `/member?sub=beneficiary`（`web/src/pages/Membership.tsx:52-55`）。

### 9.18 会员中心欢迎页与面包屑（新增）
- 路由：`/member`（需登录）
- 默认欢迎块：无 `sub` 参数时显示欢迎块，文案“{fullName} 先生/女士，您好! 欢迎您，铁路旅客积分网站”，文件 `web/src/pages/my/sections/Welcome.tsx`，样式位于 `web/src/pages/member-center.css:.mc-welcome`。
- 面包屑：左侧房子图标返回 `/` 或会员入口；会员中心页面内的房子返回 `/member`，随后显示分支与子页标题（`web/src/pages/my/MemberCenter.tsx:47-57`）。

### 9.19 登录重定向与查询参数保留（改动）
- 受保护路由：`/my/*`、`/member` 等未登录跳转 `/login`。
- 重定向保留查询：登录成功后返回来源路径，包含查询字符串，如 `/member?sub=beneficiary`（`web/src/components/ProtectedRoute.tsx:8-9` 与 `web/src/pages/Login.tsx:27-30`）。

### 9.20 受让人管理（新增）
- 入口与路由：
  - 从会员页悬停选择“受让人管理”进入 `/member?sub=beneficiary`。
  - 内容区域内：通过查询参数 `mode` 切换子流程：
    - 列表：`/member?sub=beneficiary`
    - 新增：`/member?sub=beneficiary&mode=add`
    - 修改：`/member?sub=beneficiary&mode=edit&id={id}`
    - 获取乘车人：`/member?sub=beneficiary&mode=pick`
- 文件结构：
  - 列表页：`web/src/pages/my/sections/BeneficiariesList.tsx`
  - 新增页：`web/src/pages/my/sections/AddBeneficiary.tsx`
  - 修改页：`web/src/pages/my/sections/EditBeneficiary.tsx`
  - 获取乘车人页：`web/src/pages/my/sections/PickPassengers.tsx`
  - 服务层：`web/src/services/beneficiaries.ts`
  - 容器：`web/src/pages/my/MemberCenter.tsx` 中根据 `sub`/`mode` 渲染（`web/src/pages/my/MemberCenter.tsx:74-76`）
- 列表页行为与样式：
  - 顶部面包屑：房子图标（蓝色）返回 `/member`；“受让人管理”蓝色文字点击刷新列表。
  - 添加按钮：橙色 `添加`，点击展开下拉：`新增受让人`、`获取乘车人`（`web/src/pages/member-center.css:.mc-add/.mc-add-menu`）。
  - 说明文字：按钮右侧灰色小字“本人默认为受让人，受让人添加上限为8人”。
  - 表格列：序号、姓名、证件类型、证件号码（遮罩：前4位+12个“*”+后3位）、手机（含区号遮罩）、生效日期、审核状态、操作。
  - 操作：非本人的行显示 🖊（修改）与 🗑（删除）；本人行不展示操作。
  - 删除流程：弹窗“删除确认 确认删除 {name} 用户吗？ 取消/确认”；确认后再弹“温馨提示 删除成功 确认”。
- 新增受让人：
  - 字段：姓名(必填)、证件类型(必填)、性别(单选)、证件号码(必填)、出生日期(必填，日历选择)、手机号(可选，区号+号码)、电子邮件(可选)。
  - 校验：证件号码按数字位数规则（居民身份证≥17且≤18；其他证件5–18）；出生日期必填；姓名≥2。
  - 成功弹窗：“温馨提示 受让人添加成功 确认”，点击确认返回列表。
  - 数量限制：除本人外最多8人，超限提示“受让人添加上限为8人”。
- 修改受让人：
  - 面包屑追加“> 修改受让人”。
  - 页面展示但不可编辑的只读字段：姓名、证件类型、性别、证件号码、出生日期。
  - 可编辑字段：手机号（区号+号码）、电子邮件。
  - 点击“确认”后弹“温馨提示 受让人修改成功 确认”，点击返回列表。
- 获取乘车人：
  - 数据来源：`passengers:{username}` 本地存储（`web/src/services/passengers.ts`）。
  - 勾选规则：本人行不可勾选；表头复选框选中除本人以外的全部；底部“取消/添加”。
  - 重复处理：若勾选的乘车人中有已存在受让人，弹窗“温馨提示 受让人{name}已经存在! 确认”，留在当前页；否则成功弹窗“温馨提示 受让人添加成功 确认”，返回列表。
  - 数量限制：合并后最多8人，超限提示并阻止添加。
  - 实现细节：服务层直接读取本地存储键 `passengers:{owner}`，避免运行时模块加载错误（`web/src/services/beneficiaries.ts:84-95`）。

### 9.22 会员中心内导航与模式清理（改动）
- 当在会员中心左侧导航切换子项（例如从“受让人管理”进入“会员须知”），应自动清理查询参数中的 `mode` 与 `id`，避免返回“受让人管理”仍处于新增/修改模式。
- 代码实现：`web/src/pages/my/MemberCenter.tsx:41-43` 在 `setSub` 中删除 `mode` 与 `id` 后再导航至 `/member?sub={key}`。
- 期望行为：离开新增/修改/获取模式后，再次进入“受让人管理”显示列表页。

### 9.23 获取乘车人重复提示（改动）
- 重复判断采用“证件号+姓名”组合，以减少误判（如同证件号但不同姓名的异常数据）。
- 重复或错误均以弹窗提示，而非页面红字。文件位置：`web/src/pages/my/sections/PickPassengers.tsx` 与 `web/src/services/beneficiaries.ts:88`。

### 9.24 受让人姓名唯一性（改动）
- 规则：同一账户下受让人姓名不可重复；无论是“新增受让人”还是“从乘车人获取”，若存在同名受让人，阻止新增并提示“受让人{name}已经存在!”。
- 实现：
  - 新增：`web/src/services/beneficiaries.ts:59-64` 使用 `name` 比较拦截重复。
  - 获取乘车人：`web/src/services/beneficiaries.ts:88` 使用 `name` 比较拦截重复。
- 验收：
  - 已存在“张三”时，再次新增“张三”或从乘车人勾选“张三”，均被阻止并提示。
- 存储与字段：
  - 键名：`beneficiaries:{username}`。
  - 字段：`id`、`owner`、`name`、`idType`、`idNo`、`gender?`、`birthDate?`、`phoneCode?`、`phoneNumber?`、`email?`、`effectiveDate?`、`verified`、`createdAt`（`YYYY-MM-DD`）。
  - 本人受让人：启动时自动同步插入本人（从用户资料复制），标记 `verified=true`，不可删除。

### 9.21 代码位置索引（会员与受让人相关）
- 欢迎块：`web/src/pages/my/sections/Welcome.tsx`
- 个人信息视图：`web/src/pages/my/sections/BasicInfo.tsx`
- 会员中心容器与渲染逻辑：`web/src/pages/my/MemberCenter.tsx:60-76`
- 会员页与悬停子项：`web/src/pages/Membership.tsx:39-61`
- 受保护路由与来源回跳：`web/src/components/ProtectedRoute.tsx:8-9`、`web/src/pages/Login.tsx:27-30`
- 受让人列表/新增/修改/获取乘车人：`web/src/pages/my/sections/*Beneficiary*.tsx`、`web/src/services/beneficiaries.ts`

### 9.25 会员须知页面（新增）
- 路径：`/member?sub=notice`（需登录）。入口：会员页“帮助中心”→“会员须知”，或会员中心左侧点击。
- 页面结构：
  - 居中标题“铁路畅行”常旅客会员服务须知；副标题为说明文；正文为编号条款，部分条款包含子项列表；右下角日期。
  - 文件位置：`web/src/pages/my/sections/Notice.tsx`，样式位于 `web/src/pages/member-center.css` 的 `.mn-*` 类。
- 验收：文本与排版符合示例；返回/前进行为正常；从其他子页返回时保持在会员须知子页。

### 9.26 关于会员页面（新增）
- 路径：`/member?sub=about_member`（需登录）。入口：会员页“帮助中心”→“关于会员”，或会员中心左侧点击。
- 页面结构：标题“会员账户”采用蓝色圆角徽章；下设“信息管理”“账户注销”两块，信息管理包含红色关键词与三条编号说明；账户注销段落中关键字红色强调。
- 文件：`web/src/pages/my/sections/AboutMember.tsx`；样式使用 `web/src/pages/member-center.css` 的 `.am-*` 与 `.em-red`。
- 验收：文本与示例一致，红色关键词与编号样式正确；从其他子页返回后保持在关于会员子页。

### 9.27 关于积分页面（新增）
- 路径：`/member?sub=about_points`（需登录）。入口：会员页“帮助中心”→“关于积分”，或会员中心左侧点击。
- 页面结构：包含蓝色徽章分区“积分累积”“积分补登”“受让人”“积分兑换（含兑换车票与其他规则）”“会员等级”，段落内关键字红色强调、编号列表展示。
- 文件：`web/src/pages/my/sections/AboutPoints.tsx`；样式沿用 `web/src/pages/member-center.css` 的 `.am-*` 与 `.em-red`。
- 规则提示：受让人数上限8人、积分有效期12个月、兑换资格首次需10000分、兑换比例100分=1元等说明均以文本呈现。
- 验收：内容与示例一致，各分区样式正确；从其他子页返回后保持在关于积分子页。

### 9.28 车票查询页交互（改动）
- 导航入口：点击“车票”下的“单程/往返”分别进入 `/results?ticketType=oneway` 或 `/results?ticketType=roundtrip`。
- 初始默认：进入查询页后自动填充但不搜索：
...
- 顶部日期标签：点击立即触发查询；日期标签始终基于当天生成连续15天，与当前查询日期无关。
- 从导航栏“车票→单程/往返”进入：默认不查询（`search=0`），需手动点击“查询”或点击日期标签触发。
 - 往返校验：返程日期仅需不早于出发日期（可同日）。实现位置：`web/src/pages/Home.tsx:55-62`、`web/src/pages/Results.tsx:263-268`。
- 技术实现：
  - 查询页使用 `search` 参数控制是否执行查询；默认导航填充参数并保持 `search=0`。
  - 文件位置：`web/src/pages/Results.tsx`（逻辑与输入交互）、`web/src/pages/results.css`（地址下拉样式）。
  - 验收：首次进入查询页列表为空；设置条件后点击“查询”才显示结果；输入框展开地址下拉可选。

### 9.29 首页查询卡片样式与交互（改动）
- 位置：`/` 首页，文件 `web/src/pages/Home.tsx` 与 `web/src/pages/home.css`。
- 布局：查询卡片为左侧方形框 `width=420px`；顶部为票种标签“单程/往返”，默认“单程”高亮。
- 默认值：出发地“北京”、到达地“成都”；日期默认当天；往返模式下显示返程日期输入并默认当天。
- 城市选择：点击输入框弹出 `datalist`，包含热门地址；选择后覆盖默认值。
- 交互：切换“往返”仅展示返程日期输入，不自动跳转；点击“查询”提交并进入 `/results`。
- 居中与分行：取消“清空选择”按钮；“学生”与“高铁动车”两项合并同一行并居中显示。样式类 `.center-row`。文件：`web/src/pages/home.css` 与 `web/src/pages/Home.tsx`。
- 默认勾选：高铁/动车默认勾选，首页查询时 `hs=1`。
 - 往返默认值：若未填写返程日期，则以出发日期作为默认返程日期参与查询；同时仍执行“不早于出发日期”的校验。实现位置：`web/src/pages/Home.tsx`。

### 9.30 首页左侧切换栏与查询卡片细节（新增）
- 左侧切换栏：包含“车票”（默认激活）与“订餐”；订餐为占位内容，后续实现。文件：`web/src/pages/Home.tsx`、样式：`web/src/pages/home.css:.side-nav`。
- 侧栏与表单连接：侧栏深蓝渐变（默认激活为白底，仅保留左侧蓝色竖边），与右侧白色查询表单紧密相连，无中缝。三等分块：车票、常用查询、订餐。样式类：`.side-nav .nav-item.active`、`.search-card`。
 - 尺寸微调：侧栏宽度缩至 150px；查询卡片最小高度 360px、内边距 12px；表单行间距与输入高度同步减小，使整体更紧凑。
- 查询卡片形状：设置为近似正方形（`width=420px`，`min-height=420px`），并采用相对定位以支持右侧悬浮的交换按钮。
- 行布局：出发地/到达地/出发日期各占一行；标签与输入在同一行显示（“出发地：框”）；交换按钮尺寸缩小并位于两行之间的垂直高度、框体右侧外沿。样式类：`.form-row`、`.label`、`.swap-float`。
- 顶部标签图标：在标签文字左侧显示圆形图标（蓝色激活、灰色非激活），激活标签下方显示蓝色下划线分隔。样式类：`.tab-dot`、`.search-tabs .tab.active::after`。
- 输入风格更新：去除右侧地点/日历图标；输入宽度限制不超过卡片内宽（标签+输入组合<=420px），避免溢出。
- 输入高度与交互：地址与日期输入高度进一步减小至 32px；地址框点击后自动选中并弹出完整地址列表（点击项覆盖并收起）；日期框点击或聚焦时立即弹出原生日历。样式类与实现：`Home.tsx` 中 focus/blur 控制，`home.css` 中 `.suggestions/.suggestion`。
 - 对齐规范：出发地、到达地、出发日期、返程日期四项均采用统一“标签：输入框”一行布局，标签宽度一致（`68px`），输入宽度一致（`<=260px`）。往返模式下返程日期与出发日期样式完全一致并同样支持点击弹出日历。
- 交换装饰：从第一/第二行输入右侧分别引出“折线”并在交汇处放置橙色交换按钮（22×22，圆形）。样式类：`.line-origin`、`.line-dest`、`.swap-float`。
- 交换按钮图标：圆形按钮内使用白色往返箭头矢量（双向箭头），通过内嵌 SVG 渲染，居中显示。样式：`.swap-float::before` 背景图。

### 10. 确认订单页（改动）
- 删除“选票与选座”标题与“票种：按乘客逐项设置”说明行。
- 乘客选择：
  - 同页可选择“受让人”或“乘车人”，但只能选择一种；混合选择时弹窗“不支持混合选择”，不勾选。
  - 数据来源：`beneficiaries:{username}` 与 `passengers:{username}`。
- 布局与检索：受让人与乘车人上下两块展示；右上角提供实时搜索框，按姓名过滤两块列表，输入“常”仅显示“常嘉欣”等匹配项。
- 明细表列顺序：序号、票种、席别、姓名、证件类型、证件号码、操作。
  - 默认存在序号1的空行（姓名与证件号为空）；勾选某人后填充该行；继续勾选则新增行。
  - 点击“×”：序号1仅清空姓名与证件号；其他行直接删除。
  - 每行票种与席别可独立设置；价格按席别和票种（学生9折）计算。
- 证件类型列：默认渲染为下拉“选择证件类型”；一旦选择后即显示为固定文本（如“居民身份证”）。
- 底部按钮：居中排列，“上一步”在左，“提交订单”在右，提交按钮橙色。
- 未选择提示：若未勾选任一乘客，点击“提交订单”弹窗“请选择乘车人”，确认后关闭弹窗。
- 座位选择弹窗：
  - 提交订单后先弹出小窗，展示每位乘客的席别与座位候选（A/B/C/D/F），样式对齐官网示意。
  - 不选择时点击“确认”随机分配座位；选择则按所选分配。
  - 订单存储包含车厢与座位号（如“3车厢 12A”）；订单中心新增“座位”列显示。
  - 代码位置：`web/src/pages/ConfirmOrder.tsx` 弹窗与分配、`web/src/pages/Orders.tsx` 座位显示。
- 代码位置：
  - 选择与冲突控制：`web/src/pages/ConfirmOrder.tsx`（`currentKind` 与行 `sourceKind`）
  - 明细表与行操作：`web/src/pages/ConfirmOrder.tsx`（`rows` 状态与 × 操作）
  - 提交订单：逐行生成订单，支持多乘客；文件同上
